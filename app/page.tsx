...
type NormObject = { key: string; table: string; category: string; number: string; name: string; intervals: NormInterval[]; source?: "848" | "sbcp"; baseLevel?: string; bimFactor?: number | null; bimRef?: string; bimGroup?: "1.1" | "1.2" | "1.3" | "1.4" | "1.5" };
...
type WorkTemplate = { id:string; name:string; works:Work[]; visibleRGroups?:string[]; updatedAt:string };
...
export default function Home() {
  ...
  const distributionSource = distributionSourceP;
  ...
  const numericValue=(key:string,value:number)=>numericDrafts[key] ?? (Number.isFinite(value)?String(value).replace(".",","):"");
  const editNumeric=(key:string,raw:string,apply:(value:number)=>void)=>{const normalized=raw.replace(/\./g,",").replace(/[^\d,\-]/g,"");setNumericDrafts(d=>({...d,[key]:normalized}));if(normalized!==""&&normalized!=="-"&&normalized!==","){const parsed=Number(normalized.replace(",","."));if(Number.isFinite(parsed))apply(parsed)}};
  const finishNumeric=(key:string,fallback:number,apply:(value:number)=>void)=>{const raw=numericDrafts[key];if(raw===""||raw==="-"||raw===",")apply(fallback);setNumericDrafts(d=>{const next={...d};delete next[key];return next})};
  const numberProps=(key:string,value:number,fallback:number,apply:(value:number)=>void)=>({type:"text" as const,inputMode:"decimal" as const,value:numericValue(key,value),onChange:(e:ChangeEvent<HTMLInputElement>)=>editNumeric(key,e.target.value,apply),onBlur:()=>finishNumeric(key,fallback,apply)});
  ...
  const pricing = useMemo(() => {
    ...
    const fallbackInterval=direct ?? selectedNorm.intervals[0];
    if(method==="unit") return {interval:fallbackInterval,base:area*unitRate,mode:"Расчёт по коммерческой ставке за м²",symbolFormula:"C = X × Sм²",formula:`${area} × ${unitRate}`,referenceX:area,kex:1};
    if(method==="labor") return {interval:fallbackInterval,base:hours*hourRate,mode:"Расчёт по трудозатратам",symbolFormula:"C = T × Sч",formula:`${hours} × ${hourRate}`,referenceX:hours,kex:1};
  ...
  },[selectedNorm,area,method,unitRate,hours,hourRate]);
  ...
  const timAvailable = selectedNorm.source !== "sbcp" || selectedNorm.bimFactor != null;
  const timEnabled = hasBim && timAvailable;
  const selectedKtim = timEnabled ? normativeKtim : { ...normativeKtim, p: 1, r: 1 };
  const effectiveKtim = selectedKtim;
  const isSbcp = selectedNorm.source === "sbcp";
  const tim854Group = selectedNorm.bimGroup ?? (selectedNorm.bimRef?.match(/таблиц[аы]\s+(1\.[1-5])/i)?.[1] as NormObject["bimGroup"] | undefined);
  const tim854Shares = tim854Group === "1.1"
    ? { p:.6, r:.4, rOnly:.6, table:"таблица 1", objects:"таблица 1.1 приложения 1" }
    : tim854Group === "1.2"
      ? { p:.5, r:.5, rOnly:.7, table:"таблица 2", objects:"таблица 1.2 приложения 1" }
      : tim854Group && ["1.3","1.4","1.5"].includes(tim854Group)
        ? { p:.4, r:.6, rOnly:.8, table:"таблица 3", objects:"таблицы 1.3–1.5 приложения 1" }
        : null;
  const stagePFactor = isSbcp ? (timEnabled && tim854Shares ? tim854Shares.p : .4) : .6;
  const regularStageRFactor = isSbcp ? (timEnabled && tim854Shares ? tim854Shares.r : .6) : .4;
  const isTimRdOnly = isSbcp && timEnabled && !!tim854Shares && !stageP && stageR;
  const stageRFactor = isTimRdOnly ? tim854Shares!.rOnly : regularStageRFactor;
  const stageSharesRef = isSbcp && timEnabled && tim854Shares
    ? `Методика № 854/пр, пункт ${tim854Group === "1.1" ? "10" : tim854Group === "1.2" ? "11" : "12"}, ${tim854Shares.table}`
    : isSbcp ? "СБЦП 81-2001-03" : "№ 848/пр";
  ...
  useEffect(() => {
    if (isSbcp) {
      ...
      setVisibleRGroups(rGroups.filter(g=>g.members.some(id=>(sbcpShares[id]??0)>0)).map(g=>g.key));
      return;
    }
    ...
    setVisibleRGroups(rGroups.filter(g=>g.members.some(id=>(rMap[id]??0)>0)).map(g=>g.key));
  ...
  const applyTemplate=(id:string)=>{const tpl=templates.find(t=>t.id===id);if(!tpl)return;setWorks(tpl.works.map(w=>({...w})));setVisibleRGroups(tpl.visibleRGroups?.length?tpl.visibleRGroups:rGroups.filter(g=>tpl.works.some(w=>w.group==="Р"&&(g.members.includes(w.id)||w.family===g.key)&&(w.enabled||w.share>0))).map(g=>g.key));setDistributionSourceP("manual");setDistributionSourceR("manual");};
  const updateTemplate=()=>{if(!selectedTemplateId)return;persistTemplates(templates.map(t=>t.id===selectedTemplateId?{...t,works:works.map(w=>({...w})),visibleRGroups:[...visibleRGroups],updatedAt:new Date().toLocaleDateString("ru-RU")}:t));};
  ...
  const toggleWork = (id: string) => {
    setDistributionSourceP("manual"); setDistributionSourceR("manual");
    setWorks(list => {
      const work=list.find(w=>w.id===id); if(!work || work.group==="П") return list.map(w=>w.id===id?{...w,enabled:!w.enabled}:w);
      const group=rGroups.find(g=>g.members.includes(id)||work.family===g.key); if(!group) return list.map(w=>w.id===id?{...w,enabled:!w.enabled}:w);
      const belongs=(w:Work)=>w.group==="Р"&&(group.members.includes(w.id)||w.family===group.key);
      const groupTotal=list.filter(w=>belongs(w)&&w.enabled).reduce((s,w)=>s+w.share,0);
      const wasEnabled=work.enabled;
      const next=list.map(w=>w.id===id?{...w,enabled:!w.enabled,share:wasEnabled?0:(list.some(x=>belongs(x)&&x.enabled)?0:w.share)}:w);
      const remaining=next.filter(w=>belongs(w)&&w.enabled);
      if(wasEnabled&&remaining.length){const remainingTotal=remaining.reduce((s,w)=>s+w.share,0);return next.map(w=>remaining.some(x=>x.id===w.id)?{...w,share:remainingTotal>0?w.share*groupTotal/remainingTotal:groupTotal/remaining.length}:w);}
      if(remaining.length===1) return next.map(w=>w.id===remaining[0].id?{...w,share:groupTotal||w.share}:w);
      return next;
    });
  };
  ...
  const setRGroupTotal=(key:string,value:number)=>{const target=Math.max(0,Math.min(200,value));setDistributionSourceP("manual");setDistributionSourceR("manual");setWorks(list=>{const group=rGroups.find(g=>g.key===key);if(!group)return list;const members=list.filter(w=>w.group==="Р"&&(group.members.includes(w.id)||w.family===key));const enabled=members.filter(w=>w.enabled);const current=enabled.reduce((s,w)=>s+w.share,0);if(!enabled.length){const first=members[0];return first?list.map(w=>w.id===first.id?{...w,enabled:true,share:target}:w):list}const factor=current>0?target/current:0;const each=current>0?0:target/enabled.length;return list.map(w=>enabled.some(x=>x.id===w.id)?{...w,share:current>0?w.share*factor:each}:w)});};
  const deleteRGroup=(key:string)=>{setDistributionSourceP("manual");setDistributionSourceR("manual");setVisibleRGroups(list=>list.filter(x=>x!==key));const group=rGroups.find(g=>g.key===key);if(group)setWorks(list=>list.map(w=>w.group==="Р"&&(group.members.includes(w.id)||w.family===key)?{...w,enabled:false}:w));};
  const restoreRGroup=(key:string)=>{setDistributionSourceP("manual");setDistributionSourceR("manual");setVisibleRGroups(list=>list.includes(key)?list:[...list,key]);};
  const addRWork = (family: string) => {
    const name=window.prompt("Наименование или марка нового комплекта РД");
    if (!name?.trim()) return;
    setVisibleRGroups(list=>list.includes(family)?list:[...list,family]);setWorks(list => [...list,{id:`custom-${Date.now()}`,group:"Р",name:name.trim(),share:0,enabled:true,family}]);
  };
  ...
  const chooseDistributionSourceBoth=(value:string)=>{setDistributionSourceP(value);setDistributionSourceR(value);if(value==="company"){const id=selectedTemplateId||templates[0]?.id;if(id)applyTemplate(id);return;}if(value==="848"&&selectedShareProfile){const p=sharesFromProfile(selectedShareProfile,"П"),r=sharesFromProfile(selectedShareProfile,"Р");setWorks(list=>list.map(w=>{const share=(w.group==="П"?p:r)[w.id]??0;return {...w,share,enabled:share>0}}));setVisibleRGroups(rGroups.filter(g=>g.members.some(id=>(r[id]??0)>0)).map(g=>g.key));}if(value==="sbcp"){setWorks(list=>list.map(w=>({...w,share:sbcpShares[w.id]??0,enabled:(sbcpShares[w.id]??0)>0})));setVisibleRGroups(rGroups.filter(g=>g.members.some(id=>(sbcpShares[id]??0)>0)).map(g=>g.key));}};
  const next = () => setStep(s => Math.min(5, s + 1));
  const back = () => setStep(s => Math.max(1, s - 1));

  return <main className="shell">
    <aside className={`sidebar ${mobileMenuOpen ? "mobileOpen" : ""}`}><div className="brand"><span className="brandMark">П</span><span>ПроектСмета</span></div><nav><button className={view==="offers"||view==="calculator"?"navItem active":"navItem"} onClick={()=>{setView("offers");setMobileMenuOpen(false)}}><span>▤</span> Коммерческие предложения</button><div className="offerNavList"><button className="newOfferNav" onClick={()=>{newCalculation();setMobileMenuOpen(false)}}>+ Новый расчёт</button>{savedOffers.map(o=><div className="offerNavRow" key={o.id}><button className="offerNavOpen" title={o.title} onClick={()=>{loadOffer(o);setMobileMenuOpen(false)}}><span>{o.title}</span><small>{o.date}</small></button><button className="offerNavDelete" title={`Удалить «${o.title}»`} aria-label={`Удалить расчёт ${o.title}`} onClick={()=>deleteOffer(o.id)}>⌫</button></div>)}</div><button className={view==="norms"?"navItem active":"navItem"} onClick={()=>{setView("norms");setMobileMenuOpen(false)}}><span>▦</span> Нормативы</button></nav><div className="sidebarBottom"><div className="storageNote"><strong>Локальное хранение</strong><small>Шаблоны и КП сохраняются в этом браузере</small></div></div></aside>{mobileMenuOpen&&<button className="mobileMenuBackdrop" aria-label="Закрыть меню" onClick={()=>setMobileMenuOpen(false)}/>} 
    <section className="workspace">
      <header className="topbar"><button className="mobileMenuButton" aria-label="Открыть меню" onClick={()=>setMobileMenuOpen(v=>!v)}>☰</button><div><span className="crumb">{view==="calculator"?"Расчёт":view==="offers"?"Коммерческие предложения":"Нормативы"}</span>{view==="calculator"&&<><span className="slash">/</span><span>{currentOfferTitle||"Новый расчёт"}</span></>}</div>{view==="calculator"&&<div className="topActions"><button className="ghost" onClick={saveDraft}>{saved ? "✓ Сохранено" : "Сохранить расчёт"}</button></div>}</header>
      {view==="offers"&&<div className="content libraryPage"><div className="libraryHead"><div><div className="eyebrow">АРХИВ</div><h1>Коммерческие предложения</h1><p>Сохранённые расчёты на этом устройстве</p></div><button className="primary" onClick={()=>{setView("calculator");setStep(1)}}>+ Новый расчёт</button></div>{savedOffers.length?<div className="offerList">{savedOffers.map(o=><article className="card offerCard" key={o.id}><div><strong>{o.title}</strong><span>{o.location||"Адрес не указан"}</span><small>{o.date}</small></div><b>{rub.format(o.total)}</b><div><button className="ghost" onClick={()=>{loadOffer(o);setMobileMenuOpen(false)}}>Открыть</button><button className="dangerLink" onClick={()=>deleteOffer(o.id)}>Удалить</button></div></article>)}</div>:<div className="card emptyState"><b>Пока нет сохранённых КП</b><p>Завершите расчёт и нажмите «Сохранить расчёт».</p></div>}</div>}
      {view==="norms"&&<div className="content libraryPage"><div className="libraryHead"><div><div className="eyebrow">НОРМАТИВНАЯ БАЗА</div><h1>Нормативы</h1><p>Документы, используемые калькулятором</p></div></div><div className="normLibrary">
        <div className="normGroupTitle"><span>Подключено к расчёту</span><strong>Действующие маршруты калькулятора</strong><small>Значения этих документов уже используются в формулах приложения.</small></div>
        ...
        <a className="card normDoc" href="/norms/prikaz-707.rtf" download><span>707/пр</span><div><strong>Об утверждении Методики определения стоимости работ по подготовке проектной документации</strong><small>Приказ Минстроя России от 01.10.2021 № 707/пр · формулы, интерполяция и экстраполяция</small></div><b>Скачать</b></a>
        <a className="card normDoc" href="/norms/sbcp-81-2001-03.rtf" download><span>СБЦП</span><div><strong>Справочник базовых цен на проектные работы для строительства «Объекты жилищно-гражданского строительства»</strong><small>СБЦП 81-2001-03 · действующие таблицы и рекомендуемый состав работ</small></div><b>Скачать</b></a>
        <a className="card normDoc" href="/norms/prikaz-854.rtf" download><span>854/пр</span><div><strong>Об утверждении Методики определения стоимости работ по подготовке проектной документации, содержащей материалы в форме информационной модели</strong><small>Приказ Минстроя России № 854/пр · применение ТИМ при расчёте по СБЦП</small></div><b>Скачать</b></a>
        <div className="card normDoc info"><span>87</span><div><strong>О составе разделов проектной документации и требованиях к их содержанию</strong><small>Постановление Правительства Российской Федерации от 16.02.2008 № 87</small></div><a href="https://www.consultant.ru/document/cons_doc_LAW_75048/" target="_blank" rel="noreferrer">Открыть ↗</a></div>
        <div className="normGroupTitle"><span>Следующий этап</span><strong>Современные нормативные затраты по другим видам объектов</strong><small>Документы добавлены для проверки и последующего подключения таблиц к калькулятору. Пока их наличие в библиотеке не означает, что соответствующий расчёт уже реализован.</small></div>
        <a className="card normDoc planned" href="https://minstroyrf.gov.ru/docs/?q=847%2F%D0%BF%D1%80" target="_blank" rel="noreferrer"><span>847/пр</span><div><strong>Нормативные затраты для сетей инженерно-технического обеспечения и объектов инфраструктуры</strong><small>Приказ Минстроя России от 28.11.2023 № 847/пр</small></div><b>Минстрой ↗</b></a>
        <a className="card normDoc planned" href="https://minstroyrf.gov.ru/docs/?q=849%2F%D0%BF%D1%80" target="_blank" rel="noreferrer"><span>849/пр</span><div><strong>Нормативные затраты для строительства объектов газовой промышленности</strong><small>Приказ Минстроя России от 28.11.2023 № 849/пр</small></div><b>Минстрой ↗</b></a>
        <a className="card normDoc planned" href="https://minstroyrf.gov.ru/docs/?q=51%2F%D0%BF%D1%80" target="_blank" rel="noreferrer"><span>51/пр</span><div><strong>Нормативные затраты для автомобильных дорог и сооружений на них</strong><small>Приказ Минстроя России от 03.02.2025 № 51/пр</small></div><b>Минстрой ↗</b></a>
        <a className="card normDoc planned" href="https://minstroyrf.gov.ru/docs/418823/" target="_blank" rel="noreferrer"><span>52/пр</span><div><strong>Нормативные затраты для объектов газораспределительных систем</strong><small>Приказ Минстроя России от 03.02.2025 № 52/пр · с учётом изменений № 267/пр</small></div><b>Минстрой ↗</b></a>
        <a className="card normDoc planned" href="https://www.minstroyrf.gov.ru/docs/418824/" target="_blank" rel="noreferrer"><span>53/пр</span><div><strong>Нормативные затраты для объектов водоснабжения, водоотведения и водоочистки</strong><small>Приказ Минстроя России от 03.02.2025 № 53/пр</small></div><b>Минстрой ↗</b></a>
        <a className="card normDoc planned" href="https://minstroyrf.gov.ru/docs/?q=907%2F%D0%BF%D1%80" target="_blank" rel="noreferrer"><span>907/пр</span><div><strong>Нормативные затраты для систем связи и технологических систем непроизводственных объектов</strong><small>Приказ Минстроя России от 12.12.2023 № 907/пр</small></div><b>Минстрой ↗</b></a>
        <div className="normGroupTitle"><span>Справочный каталог</span><strong>Отраслевые СБЦП</strong><small>Минстрой публикует отдельные справочники для энергетики, атомной отрасли, промышленности, транспорта и других объектов. Перед применением конкретного СБЦП необходимо проверить его статус в федеральном реестре и отсутствие более новых нормативных затрат.</small></div>
        <a className="card normDoc directory" href="https://minstroyrf.gov.ru/trades/tsenoobrazovanie/" target="_blank" rel="noreferrer"><span>СБЦП</span><div><strong>Официальный каталог справочников базовых цен Минстроя России</strong><small>Объекты энергетики, городские инженерные сооружения и коммуникации, промышленность и другие отрасли</small></div><b>Каталог ↗</b></a>
        <a className="card normDoc directory" href="https://minstroyrf.gov.ru/trades/tsenoobrazovanie/" target="_blank" rel="noreferrer"><span>АТОМ</span><div><strong>СБЦП «Объекты атомной энергетики» и «Объекты использования атомной энергии»</strong><small>Именно с объектами таблицы 1.2 Методики № 854/пр связано распределение П 50% / Р 50%; только Р — 70%</small></div><b>Каталог ↗</b></a>
      </div></div>}
      {view==="calculator"&&<div className="content">
        ...
        {step === 1 && <section className="routeCard card"><div><span className="sourceBadge">Автоматический выбор нормативного маршрута</span><h2>{isSbcp ? "СБЦП 81-2001-03 → правила 707/пр → при ТИМ 854/пр → индекс" : "848/пр → правила 707/пр → приложение 2 к 848/пр → индекс"}</h2><p>{isSbcp ? "Объект отсутствует в действующей номенклатуре № 848/пр и найден в неотменённых таблицах 33–38 СБЦП." : "Объект содержится в таблицах 3.1–3.17 № 848/пр; исключённые таблицы СБЦП параллельно не применяются."}</p></div><label className={!timAvailable ? "bimToggle unavailable" : timEnabled ? "bimToggle on" : "bimToggle"}><input type="checkbox" checked={timEnabled} disabled={!timAvailable} onChange={e => setHasBim(e.target.checked)}/><span><strong>Документация в форме информационной модели (ТИМ/BIM)</strong><small>{!timAvailable ? `Автоматическое применение недоступно: ${selectedNorm.bimRef}` : timEnabled ? (isSbcp ? `КИМ ${selectedKtim.p.toFixed(2)}: ${selectedNorm.bimRef}` : `КТИМ из приложения 2 к № 848/пр: П ${selectedKtim.p.toFixed(2)}, Р ${selectedKtim.r.toFixed(2)}`) : "ТИМ отключена: коэффициенты П = 1,00; Р = 1,00"}</small></span></label><div className="timHelp"><div className="timHelpHead"><strong>Как ТИМ/BIM влияет на расчёт</strong><span>{isSbcp ? "Методика № 854/пр" : "Приказ № 848/пр, приложение 2"}</span></div><div className="timRoute"><span>Базовая цена</span><b>→</b><span>П {Math.round(stagePFactor*100)}% / Р {Math.round(stageRFactor*100)}%</span><b>→</b><span>{isSbcp ? (timAvailable ? `КИМ ${selectedKtim.p.toFixed(2)}` : "КИМ не назначен") : `КТИМ П ${selectedKtim.p.toFixed(2)} / Р ${selectedKtim.r.toFixed(2)}`}</span><b>→</b><span>Индекс</span></div><ul>{isSbcp ? <>{timEnabled && tim854Shares ? <><li>Для объектов из {tim854Shares.objects} Методика № 854/пр устанавливает доли <strong>П {Math.round(tim854Shares.p*100)}% / Р {Math.round(tim854Shares.r*100)}%</strong>.</li><li>Если заказывается только рабочая документация в форме информационной модели на основе утверждённой обычной ПД, доля Р составляет <strong>{Math.round(tim854Shares.rOnly*100)}%</strong> общей цены П + Р.</li><li>После определения доли стадии применяется КИМ {selectedKtim.p.toFixed(2)}: {selectedNorm.bimRef}.</li></> : timAvailable ? <li>ТИМ выключена: используется обычное распределение СБЦП П 40% / Р 60%, КИМ принят равным 1,00.</li> : <li>Для выбранного объекта прямое соответствие таблицам 1.1–1.5 и коэффициент КИМ в № 854/пр не установлены. Автоматически применять Методика № 854/пр нельзя.</li>}<li>Проценты разделов ПД и РД берутся из соответствующих таблиц СБЦП и не заменяются распределением стадий по № 854/пр.</li></> : <><li>ТИМ <strong>не меняет</strong> доли стадий П 60% / Р 40% и проценты разделов документации.</li><li>К стоимости П и Р отдельно применяются коэффициенты приложения 2: П {selectedKtim.p.toFixed(2)}, Р {selectedKtim.r.toFixed(2)}.</li><li>При снятой галочке оба коэффициента принимаются равными 1,00.</li></>}</ul></div><div className="legacyRoute"><strong>Выбранный источник</strong><span>{isSbcp ? `${selectedNorm.table}, пункт ${selectedNorm.number}; цены на 01.01.2001; текущие доли П ${Math.round(stagePFactor*100)}%, Р ${Math.round(stageRFactor*100)}%.` : `№ 848/пр, таблица ${selectedNorm.table}, пункт ${selectedNorm.number}; цены на 01.01.2021.`}</span></div></section>}
        ...
        {showDetails && <div className="detailsOverlay" onClick={() => setShowDetails(false)}><section className="detailsModal card" onClick={e => e.stopPropagation()}><button className="detailsClose" onClick={() => setShowDetails(false)}>×</button><div className="eyebrow">ПРОВЕРОЧНЫЙ ЛИСТ</div><h2>Подробно о расчёте</h2><p className="detailsLead">Все множители приведены в порядке применения. Расчёт можно повторить вручную.</p>
<div className="auditFormula"><span>{method === "normative" ? "Базовая цена" : method === "unit" ? "Стоимость по ставке за м²" : "Стоимость по трудозатратам"}</span><strong>{pricing.formula} = {rub.format(calc.base)}</strong><small>{method === "normative" ? `${isSbcp ? "СБЦП 81-2001-03" : "№ 848/пр"}, ${selectedNorm.table}, пункт ${selectedNorm.number}; уровень цен ${selectedNorm.baseLevel}` : method === "unit" ? "Площадь и коммерческая ставка введены пользователем" : "Трудоёмкость и средняя часовая ставка введены пользователем"}</small></div>
<div className="auditRows">
<div><b>Способ определения</b><span>{pricing.mode}</span><small>{method === "normative" ? "Методика № 707/пр, пункт 131, формулы 8.2–8.5" : "Внутренний коммерческий метод организации"}</small></div>
<div><b>{method === "labor" ? "Трудоёмкость T" : "Показатель X"}</b><span>{method === "labor" ? `${hours.toLocaleString("ru-RU")} чел.-ч` : `${area.toLocaleString("ru-RU")} ${method === "unit" ? "м²" : selectedInterval?.unit ?? "ед."}`}</span><small>{method === "labor" ? `Средняя ставка ${rub.format(hourRate)} за час` : method === "unit" ? `Ставка ${rub.format(unitRate)} за м²` : "Натуральный показатель введён пользователем"}</small></div>
<div><b>Стадия П</b><span>{method === "normative" ? `${Math.round(stagePFactor*100)}% × КТИМ ${selectedKtim.p.toFixed(2)} × индекс ${activeIndex.toFixed(2)} × состав ${(calc.pScope*100).toFixed(1)}%` : `${Math.round(stagePFactor*100)}% × состав ${(calc.pScope*100).toFixed(1)}% × Ксл ${complexity.toFixed(2)} × Кср ${urgency.toFixed(2)}`}</span><small>{method === "normative" ? (isSbcp ? stageSharesRef : `№ 848/пр, приложение 2, пункт ${selectedKtim.number}`) : "Доля стадии и коэффициенты организации"}</small></div>
<div><b>Стадия Р{isTimRdOnly ? " (заказывается без П)" : ""}</b><span>{method === "normative" ? `${Math.round(stageRFactor*100)}% × КТИМ ${selectedKtim.r.toFixed(2)} × индекс ${activeIndex.toFixed(2)} × состав ${(calc.rScope*100).toFixed(1)}%` : `${Math.round(stageRFactor*100)}% × состав ${(calc.rScope*100).toFixed(1)}% × Ксл ${complexity.toFixed(2)} × Кср ${urgency.toFixed(2)}`}</span><small>{method === "normative" ? (isSbcp ? stageSharesRef : `№ 848/пр, приложение 2, пункт ${selectedKtim.number}`) : "Доля стадии и коэффициенты организации"}</small></div>
<div><b>Распределение разделов</b><span>{distributionSource === "848" ? "№ 848/пр, приложение 1, таблица для выбранной группы" : distributionSource === "sbcp" ? "СБЦП 81-2001-03, таблицы 41 и 42" : "Ручное распределение / шаблон организации"}</span><small>Влияет на состав и внутреннюю раскладку стоимости работ</small></div>
<div><b>Индекс пересчёта</b><span>{method === "normative" ? `${activeIndex.toFixed(2)} · ${isSbcp ? "введён вручную" : priceLevels[quarter]?.label ?? "Будущий квартал"}` : "Не применяется"}</span><small>{method === "normative" ? `Переход от уровня цен ${selectedNorm.baseLevel}` : "Цена непосредственно определяется выбранным коммерческим методом"}</small></div>
<div><b>{method === "normative" ? "Дополнительные нормативные условия" : "Коэффициенты организации"}</b><span>{method === "normative" ? "Не применены" : `Сложность ${complexity.toFixed(2)} · срочность ${urgency.toFixed(2)}`}</span><small>{method === "normative" ? "Стеснённость и иные факторы включаются только при подтверждённом основании" : "Не являются коэффициентами приказов Минстроя и устанавливаются организацией"}</small></div>
</div>
<div className="methodSources"><h3>{method === "normative" ? "Нормативная основа" : "Основание коммерческого расчёта"}</h3>{method === "normative" ? <><a href="https://www.minstroyrf.gov.ru/docs/355755/" target="_blank" rel="noreferrer">Приказ Минстроя № 848/пр ↗</a><a href="https://minstroyrf.gov.ru/upload/iblock/0e3/Metodika-opredeleniya-stoimosti-rabot-po-podgotovke-proektnoy-dokumentatsii.pdf" target="_blank" rel="noreferrer">Методика № 707/пр ↗</a><a href="https://www.minstroyrf.gov.ru/docs/11900/" target="_blank" rel="noreferrer">СБЦП 81-2001-03 ↗</a><a href="https://base.garant.ru/400375659/" target="_blank" rel="noreferrer">Методика № 854/пр ↗</a><p>№ 707/пр устанавливает общий порядок определения стоимости; № 848/пр содержит нормативные затраты для жилищно-гражданских объектов.</p></> : <p>{method === "unit" ? "Стоимость определяется произведением площади объекта на внутреннюю коммерческую ставку за квадратный метр." : "Стоимость определяется произведением принятой трудоёмкости на среднюю стоимость человеко-часа."} Нормативные индексы и коэффициенты ТИМ в этой формуле не применяются.</p>}</div></section></div>}
        {step === 3 && method === "normative" && <div className="quarterBar card"><label><span>Уровень цен</span><select value={quarter} onChange={e => chooseQuarter(e.target.value)}>{Object.entries(priceLevels).map(([key,v])=><option key={key} value={key}>{v.label} — {isSbcp ? v.i2001.toFixed(2) : v.i2021.toFixed(2)}</option>)}<option value="future">Будущий квартал — индекс вручную</option></select><small>{quarter === "future" ? "Официальный индекс ещё не опубликован; значение задаётся вручную." : priceLevels[quarter]?.letter}</small></label><label className={indexMismatch ? "indexField mismatch" : "indexField"}><span>Индекс к {isSbcp ? "01.01.2001" : "01.01.2021"}</span><input step=".01" {...numberProps("index",activeIndex,expectedIndex??1,v=>{isSbcp?setSbcpIndex(v):setIndex(v)})}/><small>{indexMismatch ? `Введено ${activeIndex.toFixed(2)}, но выбранному кварталу соответствует ${expectedIndex?.toFixed(2)}` : quarter === "future" ? "Проверьте значение после публикации очередного письма Минстроя." : `Значение соответствует ${priceLevels[quarter]?.label}`}</small><a href="https://minstroyrf.gov.ru/trades/tsenoobrazovanie/" target="_blank" rel="noreferrer">Источник: Минстрой России · Индексы изменения сметной стоимости ↗</a></label></div>}
        {step === 3 && method === "normative" && <div className="normBreakdown card"><div><strong>Как собрана нормативная цена</strong><span>Все применённые значения показаны отдельно</span></div><dl><div><dt>а = {selectedInterval?.a ?? 0} тыс. ₽; b = {selectedInterval?.b ?? 0}</dt><dd>{isSbcp ? "СБЦП 81-2001-03" : "Приказ № 848/пр"}, {selectedNorm.table}, пункт {selectedNorm.number}</dd></div><div><dt>X = {area.toLocaleString("ru-RU")} {selectedInterval?.unit}</dt><dd>Натуральный показатель, введённый на шаге 1</dd></div><div><dt>П = {Math.round(stagePFactor*100)}%; Р = {Math.round(stageRFactor*100)}%{isTimRdOnly ? " (только Р)" : ""}</dt><dd>{stageSharesRef}</dd></div><div><dt>{timEnabled ? `ТИМ: П ${selectedKtim.p.toFixed(2)}; Р ${selectedKtim.r.toFixed(2)}` : "ТИМ не применяется"}</dt><dd>{isSbcp ? selectedNorm.bimRef : "№ 848/пр, приложение 2"}</dd></div><div><dt>Индекс = {activeIndex.toFixed(2)}</dt><dd>Пересчёт от уровня цен {selectedNorm.baseLevel}</dd></div></dl><p><strong>Методика № 707/пр</strong> задаёт общий порядок расчёта. Источник цены выбран автоматически: сначала № 848/пр, а для отсутствующих там объектов — только действующие таблицы 33–38 СБЦП.</p></div>}
        ...
        {step === 1 && <div className="flowGrid"><section className="card flowCard"><div className="cardHead"><div><h2>Исходные данные объекта</h2><p>Показатель X и его единица определяются выбранной строкой № 848/пр</p></div></div><div className="sectionTitle">Основные сведения</div><div className="fields"><label><span>Наименование объекта</span><input value={title} onChange={e => setTitle(e.target.value)}/></label><label><span>Адрес / местоположение</span><input value={location} onChange={e => setLocation(e.target.value)}/></label><label><span>Натуральный показатель X</span><div className="inputUnit"><input {...numberProps("area",area,0,setArea)}/><b>{selectedInterval?.unit ?? "ед."}</b></div><small>Допустимый интервал: {selectedInterval?.label}</small></label></div></section><FlowSummary step={step} calc={calc} method={method} /></div>}
        ...
        {step === 2 && <div className="flowGrid"><section className="card flowCard">
          ...
          {isMultiResidential && <div className="floorProfile"><label><span>Этажность многоквартирного дома</span><input min="1" {...numberProps("floors",floors,1,v=>setFloors(Math.max(1,v)))}/></label><div><strong>{floors <= 9 ? "До 9 этажей включительно" : "10 этажей и более"}</strong><small>Автоматически выбран профиль {floors <= 9 ? "3.1" : "3.2"} таблицы 1 приложения 1 к № 848/пр. Газоснабжение показывается только при наличии процента в выбранной строке.</small></div></div>}
          {showInternal && <div className="internalLayout">
            <div className="distributionBar singleSource"><label><span>Источник процентов ПД и РД</span><select value={distributionSource==="manual"?"manual":(isSbcp?"sbcp":"848")} onChange={e=>chooseDistributionSourceBoth(e.target.value)}><option value={isSbcp?"sbcp":"848"}>{isSbcp?"Нормативное · СБЦП 81-2001-03, таблицы 41 и 42":"Нормативное · № 848/пр, приложение 1"}</option><option value="manual">Ручное распределение</option></select></label></div>
            {distributionSource==="manual"&&<div className="templateManager"><div><label><span>Сохранённый шаблон</span><select value={selectedTemplateId} onChange={e=>setSelectedTemplateId(e.target.value)}><option value="">Выберите шаблон</option>{templates.map(t=><option key={t.id} value={t.id}>{t.name}</option>)}</select></label><button className="primary" disabled={!selectedTemplateId} onClick={()=>applyTemplate(selectedTemplateId)}>Применить</button><button className="ghost" disabled={!selectedTemplateId} onClick={updateTemplate}>Обновить</button><button className="dangerLink" disabled={!selectedTemplateId} onClick={deleteTemplate}>Удалить</button></div><div><input value={templateName} onChange={e=>setTemplateName(e.target.value)} placeholder="Название нового шаблона"/><button className="primary" disabled={!templateName.trim()} onClick={createTemplate}>Сохранить текущий состав</button></div><small>Сначала выберите шаблон, затем нажмите «Применить». Шаблон хранит состав ПД, РД, видимые группы и проценты.</small></div>}
            ...
            <div className="workColumns mappedColumns">
              <div><div className="workHeader"><div><strong>Разделы ПД по постановлению № 87</strong><small>Итог можно менять — составляющие масштабируются один раз пропорционально</small></div><div className={Math.abs(works.filter(w=>w.group==="П"&&w.enabled).reduce((s,w)=>s+w.share,0)-100)<.05 ? "totalMarker ok editable" : "totalMarker bad editable"}><input aria-label="Общий процент ПД" step=".1" {...numberProps("p-total",Number(works.filter(w=>w.group==="П"&&w.enabled).reduce((s,w)=>s+w.share,0).toFixed(2)),0,v=>setGroupTotal("П",v))}/><b>%</b></div></div>{works.filter(w=>w.group==="П"&&w.enabled).map(w=><label className={`workItem family-${familyByWork[w.id] ?? "other"}`} key={w.id}><input type="checkbox" checked={w.enabled} onChange={()=>toggleWork(w.id)}/><span>{w.name}</span><div className="percentEdit"><input step=".1" {...numberProps(`p-${w.id}`,Number(w.share.toFixed(2)),0,v=>updateShare(w.id,v))}/><b>%</b></div></label>)}</div>
              <div><div className="workHeader"><div><strong>Группы и комплекты РД</strong><small>Карточка удаляется только кнопкой-корзиной</small></div><div className={Math.abs(works.filter(w=>w.group==="Р"&&w.enabled).reduce((s,w)=>s+w.share,0)-100)<.05 ? "totalMarker ok editable" : "totalMarker bad editable"}><input aria-label="Общий процент РД" step=".1" {...numberProps("r-total",Number(works.filter(w=>w.group==="Р"&&w.enabled).reduce((s,w)=>s+w.share,0).toFixed(2)),0,v=>setGroupTotal("Р",v))}/><b>%</b></div></div>{rGroups.filter(g=>visibleRGroups.includes(g.key)).map(g=>{const items=works.filter(w=>w.group==="Р"&&(g.members.includes(w.id)||w.family===g.key));const selectedCount=items.filter(w=>w.enabled).length;const total=items.filter(w=>w.enabled).reduce((s,w)=>s+w.share,0);const normativeTarget=g.members.reduce((s,id)=>s+(normativeRTargets[id]??0),0);const organizational=distributionSource==="manual"||distributionSource==="company";const target=organizational?total:normativeTarget;const distributed=!!distributedGroups[g.key];return <section className={`rdGroup family-${g.key}`} key={g.key}><div className="rdGroupHead"><div><strong>{g.name}</strong><small>{g.marks}</small></div><div className="rdGroupHeadActions">{organizational?<div className="groupTotalEdit"><input aria-label={`Общий процент ${g.name}`} step=".1" {...numberProps(`rg-${g.key}`,Number(total.toFixed(2)),0,v=>setRGroupTotal(g.key,v))}/><b>%</b></div>:<b>{target.toFixed(2)}%</b>}<button className="trashGroup" title="Удалить группу" aria-label={`Удалить ${g.name}`} onClick={()=>deleteRGroup(g.key)}>⌫</button></div></div><div className="rdMarks">{items.map(w=><div className="rdMark" key={w.id}><input type="checkbox" checked={w.enabled} onChange={()=>toggleWork(w.id)}/>{w.id.startsWith("custom-")?<input className="markName" value={w.name} onChange={e=>setWorks(list=>list.map(x=>x.id===w.id?{...x,name:e.target.value}:x))}/>:<span>{w.name}</span>}{w.enabled&&<div className="percentEdit"><input step=".1" disabled={selectedCount<=1} {...numberProps(`r-${w.id}`,Number(w.share.toFixed(2)),0,v=>updateShare(w.id,v))}/><b>%</b></div>}{w.id.startsWith("custom-")&&<button className="removeMark" onClick={()=>removeRWork(w.id)}>×</button>}</div>)}</div><div className="rdGroupActions singleAction"><button onClick={()=>addRWork(g.key)}>+ Добавить комплект</button></div>{selectedCount>1&&<small className={organizational||Math.abs(total-target)<.01?"distributionOk":"distributionBad"}>Сумма комплектов: {total.toFixed(2)}%{!organizational&&` · норматив группы ${target.toFixed(2)}%`}</small>}</section>})}{visibleRGroups.length<rGroups.length&&<div className="restoreGroups"><span>Удалённые группы:</span>{rGroups.filter(g=>!visibleRGroups.includes(g.key)).map(g=><button key={g.key} onClick={()=>restoreRGroup(g.key)}>+ {g.name}</button>)}</div>}</div>
            </div>
        ...
        {step === 3 && <div className="flowGrid"><section className="card flowCard"><div className="cardHead"><div><h2>Метод и параметры расчёта</h2><p>Настройте нормативную основу и коммерческие факторы</p></div></div><div className="methodTabs">{[["normative","Нормативные затраты"],["unit","Ставка за м²"],["labor","Трудозатраты"]].map(([id,name]) => <button key={id} className={method === id ? "on" : ""} onClick={() => setMethod(id as Method)}>{name}</button>)}</div>{method === "normative" && <div className="normative"><div className="docIcon">§</div><div><strong>{isSbcp ? "СБЦП 81-2001-03" : "Приказ Минстроя России № 848/пр"}</strong><p>{selectedNorm.table}, пункт {selectedNorm.number} · база {selectedNorm.baseLevel}</p></div><span className="valid">✓ Применим</span></div>}{method === "unit" && <div className="methodPanel"><label><span>Коммерческая ставка</span><div className="inputUnit"><input {...numberProps("unitRate",unitRate,0,setUnitRate)}/><b>₽/м²</b></div></label><div className="formula"><span>Расчёт</span><code>{area.toLocaleString("ru-RU")} м² × {rub.format(unitRate)} = <strong>{rub.format(calc.base)}</strong></code></div><p className="warning">Это внутренняя коммерческая ставка, а не норматив Минстроя.</p></div>}{method === "labor" && <div className="fields"><label><span>Трудоёмкость</span><div className="inputUnit"><input {...numberProps("hours",hours,0,setHours)}/><b>чел.-ч</b></div></label><label><span>Средняя ставка</span><div className="inputUnit"><input {...numberProps("hourRate",hourRate,0,setHourRate)}/><b>₽/ч</b></div></label></div>}<section className="calculationDetails"><div className="calculationDetailsHead"><div><span>ПОДРОБНОСТИ РАСЧЁТА</span><h2>{method === "normative" ? "Нормативные затраты" : method === "unit" ? "Коммерческая ставка за м²" : "Расчёт по трудозатратам"}</h2></div><small>{pricing.mode}</small></div><div className="calculationEquation"><span>{pricing.symbolFormula}</span><code>{pricing.formula} = <strong>{rub.format(calc.base)}</strong></code></div><div className="calculationVariables">{method === "normative" ? <><div><b>a</b><span>{selectedInterval?.a ?? 0} тыс. ₽ — постоянная из нормативной таблицы</span></div><div><b>b</b><span>{selectedInterval?.b ?? 0} тыс. ₽/ед. — коэффициент нормативной строки</span></div><div><b>X</b><span>{area.toLocaleString("ru-RU")} {selectedInterval?.unit ?? "ед."} — показатель объекта</span></div><div><b>1 000</b><span>перевод из тысяч рублей в рубли</span></div></> : method === "unit" ? <><div><b>X</b><span>{area.toLocaleString("ru-RU")} м² — площадь объекта</span></div><div><b>Sм²</b><span>{rub.format(unitRate)} за м² — ставка организации</span></div></> : <><div><b>T</b><span>{hours.toLocaleString("ru-RU")} чел.-ч — трудоёмкость</span></div><div><b>Sч</b><span>{rub.format(hourRate)} за час — средняя ставка</span></div></>}</div><p>{method === "normative" ? `Основание: ${isSbcp ? "СБЦП 81-2001-03" : "№ 848/пр"}, ${selectedNorm.table}, пункт ${selectedNorm.number}; порядок расчёта — Методика № 707/пр.` : "Внутренний коммерческий метод организации. Значения ставки и трудоёмкости задаются пользователем и не являются нормативами Минстроя."}</p></section><div className="sectionTitle">Повторяемость и сложность</div><div className="fields"><label><span>Коэффициент повторной секции</span><select value={repeat} onChange={e => setRepeat(+e.target.value)} disabled={sections === 1}><option value="1">Уникальные — 1,00</option><option value="0.5">Частично повторные — 0,50</option><option value="0.3">Зеркальные — 0,30</option><option value="0.25">Повторные — 0,25</option></select><small>{sections} секц. · итоговый фактор {calc.repeatFactor.toFixed(3)}</small></label><label><span>Техническая сложность</span><select value={complexity} onChange={e => setComplexity(+e.target.value)}><option value="1">Обычная — 1,00</option><option value="1.15">Повышенная — 1,15</option><option value="1.3">Высокая — 1,30</option><option value="1.5">Уникальный объект — 1,50</option></select></label><label><span>Срочность</span><select value={urgency} onChange={e => setUrgency(+e.target.value)}><option value="1">Стандартная — 1,00</option><option value="1.1">Ускоренная — 1,10</option><option value="1.2">Срочная — 1,20</option></select></label><label><span>НДС</span><select value={vat} onChange={e => setVat(+e.target.value)}><option value="0">Без НДС</option><option value="5">5%</option><option value="7">7%</option><option value="20">20%</option></select></label></div></section><FlowSummary step={step} calc={calc} method={method} /></div>}
        {step === 4 && <div className="resultLayout"><section className="proposal"><div className="proposalTop"><div><span>КОММЕРЧЕСКОЕ ПРЕДЛОЖЕНИЕ</span><h2>{title}</h2><p>{location}</p></div><div className="proposalNumber">КП № 26-216<small>08.08.2026</small></div></div><div className="proposalFacts"><div><span>Тип объекта</span><strong>{selectedNorm.name}</strong></div><div><span>Площадь</span><strong>{area.toLocaleString("ru-RU")} {selectedInterval?.unit ?? "ед."}</strong></div><div><span>Стадийность</span><strong>{[sketch&&"ЭП",stageP&&"П",stageR&&"Р"].filter(Boolean).join(" + ")}</strong></div><div><span>Метод</span><strong>{method === "normative" ? (isSbcp ? "СБЦП 81-2001-03" : "№ 848/пр") : method === "unit" ? "Ставка за м²" : "Трудозатраты"}</strong></div></div><table className="resultTable"><thead><tr><th>№</th><th>Наименование работ</th><th>Основание</th><th>Стоимость</th></tr></thead><tbody>{stageP&&<tr><td>1</td><td><strong>Проектная документация</strong></td><td>{Math.round(stagePFactor*100)}% · КТИМ {selectedKtim.p.toFixed(2)}</td><td>{rub.format(calc.p)}</td></tr>}{stageR&&<tr><td>2</td><td><strong>Рабочая документация</strong></td><td>{Math.round(stageRFactor*100)}% · КТИМ {selectedKtim.r.toFixed(2)}</td><td>{rub.format(calc.r)}</td></tr>}{sketch&&<tr><td>3</td><td><strong>Эскизный проект</strong><small>Стоимость определена как настраиваемый процент от суммы стадий П и Р. Рекомендуется проверка по трудозатратам.</small></td><td>{sketchPercent}% от суммы П + Р</td><td>{rub.format(calc.ep)}</td></tr>}</tbody><tfoot><tr><td colSpan={3}>Итого без НДС</td><td>{rub.format(calc.subtotal)}</td></tr><tr><td colSpan={3}>НДС {vat}%</td><td>{rub.format(calc.tax)}</td></tr><tr className="final"><td colSpan={3}>Итого к оплате</td><td>{rub.format(calc.total)}</td></tr></tfoot></table><div className="assumptions"><h3>Параметры расчёта</h3>{method === "normative" ? <><p>Показатель X = {area.toLocaleString("ru-RU")} {selectedInterval?.unit ?? "ед."} · {selectedNorm.table}, пункт {selectedNorm.number} · КТИМ П {selectedKtim.p.toFixed(2)} / Р {selectedKtim.r.toFixed(2)} · индекс {activeIndex.toFixed(2)}.</p><p className="muted">Стоимость определена нормативным методом. Состав разделов и применённые коэффициенты необходимо проверить перед использованием в договорной работе.</p></> : method === "unit" ? <><p>Площадь X = {area.toLocaleString("ru-RU")} м² · ставка Sм² = {rub.format(unitRate)} за м² · исходная стоимость {rub.format(calc.base)}.</p><p className="muted">Коммерческий расчёт по внутренней ставке организации. Нормативные индексы и коэффициенты ТИМ не применяются.</p></> : <><p>Трудоёмкость T = {hours.toLocaleString("ru-RU")} чел.-ч · средняя ставка Sч = {rub.format(hourRate)} за час · исходная стоимость {rub.format(calc.base)}.</p><p className="muted">Коммерческий расчёт по трудозатратам организации. Нормативные индексы и коэффициенты ТИМ не применяются.</p></>}</div><section className="internalMoney noPrint"><div><h3>Внутренняя денежная раскладка</h3><p>Служебный расчёт организации. В печатную версию для заказчика не включается.</p></div><div className="moneyColumns"><div><strong>Проектная документация</strong>{works.filter(w=>w.group==="П"&&w.enabled).map(w=><p key={w.id}><span>{w.name} · {w.share.toFixed(2)}%</span><b>{rub.format(calc.p*w.share/100)}</b></p>)}</div><div><strong>Рабочая документация</strong>{rGroups.map(g=>{const selected=works.filter(w=>w.group==="Р"&&w.enabled&&(g.members.includes(w.id)||w.family===g.key));const share=selected.reduce((s,w)=>s+w.share,0);return share>0?<div className="moneyGroup" key={g.key}><p><span>{g.name} · {share.toFixed(2)}%</span><b>{rub.format(calc.r*share/100)}</b></p><div className="moneySubsections">{selected.map(w=><div className="moneySubRow" key={w.id}><span>{w.name} · {w.share.toFixed(2)}%</span><b>{rub.format(calc.r*w.share/100)}</b></div>)}</div></div>:null})}</div></div></section></section><aside className="resultActions card"><div className="successIcon">✓</div><h2>Расчёт готов</h2><p>Проверьте состав работ и итоговую стоимость.</p><div className="bigResult">{rub.format(calc.total)}<small>с НДС {vat}%</small></div><button className="primary wide" onClick={() => window.print()}>Печать / сохранить PDF</button><button className="ghost wide" onClick={saveDraft}>Сохранить расчёт</button><button className="linkButton wide" onClick={() => setStep(3)}>← Вернуться к расчёту</button></aside></div>}
