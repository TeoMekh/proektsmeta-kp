...
export default function Home() {
  ...
  const createTemplate=()=>{const name=templateName.trim();if(!name)return;const item={id:`tpl-${Date.now()}`,name,works:works.map(w=>({...w})),visibleRGroups:[...visibleRGroups],updatedAt:new Date().toLocaleDateString("ru-RU")};persistTemplates([...templates,item]);setSelectedTemplateId(item.id);setTemplateName("");setDistributionSourceP("company");setDistributionSourceR("company");};
  const applyTemplate=(id:string)=>{const tpl=templates.find(t=>t.id===id);setSelectedTemplateId(id);if(!tpl)return;setWorks(tpl.works.map(w=>({...w})));setVisibleRGroups(tpl.visibleRGroups?.length?tpl.visibleRGroups:rGroups.filter(g=>tpl.works.some(w=>w.group==="Р"&&(g.members.includes(w.id)||w.family===g.key)&&(w.enabled||w.share>0))).map(g=>g.key));setDistributionSourceP("company");setDistributionSourceR("company");};
      ...
      {view==="calculator"&&<div className="content">
        ...
        {step === 2 && <div className="flowGrid"><section className="card flowCard">
          ...
          {showInternal && <div className="internalLayout">
            <div className="distributionBar singleSource"><label><span>Источник процентов ПД и РД</span><select value={distributionSource} onChange={e=>chooseDistributionSourceBoth(e.target.value)}><option value={isSbcp?"sbcp":"848"}>{isSbcp?"СБЦП 81-2001-03 · таблицы 41 и 42":"№ 848/пр · приложение 1"}</option><option value="company" disabled={!templates.length}>Шаблон организации{templates.length?` · ${templates.length}`:" · сначала сохраните шаблон"}</option><option value="manual">Ручное распределение</option></select></label></div>
            {(distributionSource==="company"||distributionSource==="manual")&&<div className="templateManager"><div><label><span>Сохранённый шаблон</span><select value={selectedTemplateId} onChange={e=>applyTemplate(e.target.value)}><option value="">Выберите шаблон</option>{templates.map(t=><option key={t.id} value={t.id}>{t.name}</option>)}</select></label><button className="ghost" disabled={!selectedTemplateId} onClick={updateTemplate}>Обновить</button><button className="dangerLink" disabled={!selectedTemplateId} onClick={deleteTemplate}>Удалить</button></div><div><input value={templateName} onChange={e=>setTemplateName(e.target.value)} placeholder="Название нового шаблона"/><button className="primary" disabled={!templateName.trim()} onClick={createTemplate}>Сохранить текущий состав</button></div><small>Шаблон применяется сразу и хранит состав ПД, РД, видимые группы и проценты.</small></div>}
