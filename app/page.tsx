  ...
  return <main className="shell">
    ...
    <section className="workspace">
      ...
      {view==="calculator"&&<div className="content">
        ...
        {step === 1 && <section className="routeCard card"><div><span className="sourceBadge">Автоматический выбор нормативного маршрута</span><h2>{isSbcp ? "СБЦП 81-2001-03 → правила 707/пр → при ТИМ 854/пр → индекс" : "848/пр → правила 707/пр → приложение 2 к 848/пр → индекс"}</h2><p>{isSbcp ? "Объект отсутствует в действующей номенклатуре № 848/пр и найден в неотменённых таблицах 33–38 СБЦП." : "Объект содержится в таблицах 3.1–3.17 № 848/пр; исключённые таблицы СБЦП параллельно не применяются."}</p></div><label className={!timAvailable ? "bimToggle unavailable" : timEnabled ? "bimToggle on" : "bimToggle"}><input type="checkbox" checked={timEnabled} disabled={!timAvailable} onChange={e => setHasBim(e.target.checked)}/><span><strong>Документация в форме информационной модели (ТИМ/BIM)</strong><small>{!timAvailable ? `Автоматическое применение недоступно: ${selectedNorm.bimRef}` : timEnabled ? (isSbcp ? `КИМ ${selectedKtim.p.toFixed(2)}: ${selectedNorm.bimRef}` : `КТИМ из приложения 2 к № 848/пр: П ${selectedKtim.p.toFixed(2)}, Р ${selectedKtim.r.toFixed(2)}`) : "ТИМ отключена: коэффициенты П = 1,00; Р = 1,00"}</small></span></label><div className="timHelp"><div className="timHelpHead"><strong>Как ТИМ/BIM влияет на расчёт</strong><span>{isSbcp ? "Методика № 854/пр" : "Приказ № 848/пр, приложение 2"}</span></div><div className="timRoute"><span>Базовая цена</span><b>→</b><span>{isSbcp ? "П 40% / Р 60%" : "П 60% / Р 40%"}</span><b>→</b><span>{isSbcp ? (timAvailable ? `КИМ ${selectedKtim.p.toFixed(2)}` : "КИМ не назначен") : `КТИМ П ${selectedKtim.p.toFixed(2)} / Р ${selectedKtim.r.toFixed(2)}`}</span><b>→</b><span>Индекс</span></div><ul>{isSbcp ? <><li>Методика № 854/пр <strong>не меняет</strong> соотношение стадий П 40% / Р 60%, установленное СБЦП.</li>{timAvailable ? <li>После разделения стоимости к П и Р применяется коэффициент КИМ: {selectedNorm.bimRef}.</li> : <li>Для выбранного объекта прямой коэффициент в № 854/пр не найден. Автоматически повышать стоимость нельзя; применение объекта-аналога возможно только вручную с обоснованием.</li>}<li>Распределение стоимости по разделам остаётся по таблицам соответствующего СБЦП.</li></> : <><li>ТИМ <strong>не меняет</strong> доли стадий П 60% / Р 40% и проценты разделов документации.</li><li>К стоимости П и Р отдельно применяются коэффициенты приложения 2: П {selectedKtim.p.toFixed(2)}, Р {selectedKtim.r.toFixed(2)}.</li><li>При снятой галочке оба коэффициента принимаются равными 1,00.</li></>}</ul></div><div className="legacyRoute"><strong>Выбранный источник</strong><span>{isSbcp ? `${selectedNorm.table}, пункт ${selectedNorm.number}; цены на 01.01.2001; П 40%, Р 60%.` : `№ 848/пр, таблица ${selectedNorm.table}, пункт ${selectedNorm.number}; цены на 01.01.2021.`}</span></div></section>}
...
<div><b>Стадия П</b><span>{method === "normative" ? `${Math.round(stagePFactor*100)}% × КТИМ ${selectedKtim.p.toFixed(2)} × индекс ${activeIndex.toFixed(2)} × состав ${(calc.pScope*100).toFixed(1)}%` : `${Math.round(stagePFactor*100)}% × состав ${(calc.pScope*100).toFixed(1)}% × Ксл ${complexity.toFixed(2)} × Кср ${urgency.toFixed(2)}`}</span><small>{method === "normative" ? (isSbcp ? selectedNorm.bimRef : `№ 848/пр, приложение 2, пункт ${selectedKtim.number}`) : "Доля стадии и коэффициенты организации"}</small></div>
<div><b>Стадия Р</b><span>{method === "normative" ? `${Math.round(stageRFactor*100)}% × КТИМ ${selectedKtim.r.toFixed(2)} × индекс ${activeIndex.toFixed(2)} × состав ${(calc.rScope*100).toFixed(1)}%` : `${Math.round(stageRFactor*100)}% × состав ${(calc.rScope*100).toFixed(1)}% × Ксл ${complexity.toFixed(2)} × Кср ${urgency.toFixed(2)}`}</span><small>{method === "normative" ? (isSbcp ? selectedNorm.bimRef : `№ 848/пр, приложение 2, пункт ${selectedKtim.number}`) : "Доля стадии и коэффициенты организации"}</small></div>
