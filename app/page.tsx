...
export default function Home() {
  ...
  const selectedInterval = selectedNorm.intervals.find(i => (i.min == null || area >= i.min) && (i.max == null || area <= i.max));
  const selectedKtim = useMemo(() => {
  ...
  const calc = useMemo(() => {
    const normBase = selectedInterval?.a != null ? (selectedInterval.a + (selectedInterval.b ?? 0) * area) * 1000 : 0;
    ...
    const p = stageP ? adjusted * .6 * pScope * (method === "normative" ? selectedKtim.p * index : 1) : 0;
    const r = stageR ? adjusted * .4 * rScope * (method === "normative" ? selectedKtim.r * index : 1) : 0;
  ...
  }, [area, complexity, fixedPrice, hourRate, hours, index, method, repeat, sections, selectedInterval, selectedKtim, sketch, stageP, stageR, unitRate, urgency, vat, works]);
