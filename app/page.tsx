...
type NormObject = { key: string; table: string; category: string; number: string; name: string; intervals: NormInterval[] };
...
export default function Home() {
  ...
  const normObjects = normative848 as NormObject[];
  ...
  const pricing = useMemo(() => {
    ...
    if(direct) return {interval:direct,base:price(direct,area),mode:"В пределах нормативного интервала",formula:`(${direct.a ?? 0} + ${direct.b ?? 0} × ${area}) × 1 000`,referenceX:area,kex:1};
    ...
    if(first && xmin != null && area < xmin){
      const half=xmin/2;
      if(area >= half){const xref=.4*xmin+.6*area;return {interval:first,base:price(first,xref),mode:"Экстраполяция по формуле 8.2",formula:`(${first.a ?? 0} + ${first.b ?? 0} × (0,4 × ${xmin} + 0,6 × ${area})) × 1 000`,referenceX:xref,kex:1};}
      const kex=Math.max(.1,area/half),xref=.4*xmin+.6*half;
      return {interval:first,base:price(first,xref)*kex,mode:"Экстраполяция по формулам 8.4–8.5",formula:`[(${first.a ?? 0} + ${first.b ?? 0} × (0,4 × ${xmin} + 0,6 × ${half})) × 1 000] × ${kex.toFixed(3)}`,referenceX:xref,kex};
    }
    if(last && max != null && area > max){const xref=.4*max+.6*area;return {interval:last,base:price(last,xref),mode:"Экстраполяция по формуле 8.3",formula:`(${last.a ?? 0} + ${last.b ?? 0} × (0,4 × ${max} + 0,6 × ${area})) × 1 000`,referenceX:xref,kex:1};}
    return {interval:first,base:price(first,area),mode:"Расчёт по единственной нормативной строке",formula:`(${first?.a ?? 0} + ${first?.b ?? 0} × ${area}) × 1 000`,referenceX:area,kex:1};
  },[selectedNorm,area]);
  ...
  const chooseNormTable = (table: string) => {
    ...
    setDistributionSourceP(first.key === "3.8-1" ? "848" : "manual"); setDistributionSourceR(first.key === "3.8-1" ? "848" : "manual");
    const interval = first.intervals[0]; if (interval?.min != null && interval?.max != null) setArea(Math.round((interval.min + interval.max) / 2));
  };
  const chooseNormObject = (key: string) => {
    const item = normObjects.find(n => n.key === key); if (!item) return;
    setNormKey(key); setTitle(item.name);
    setDistributionSourceP(key === "3.8-1" ? "848" : "manual"); setDistributionSourceR(key === "3.8-1" ? "848" : "manual");
    const interval = item.intervals[0]; if (interval?.min != null && interval?.max != null) setArea(Math.round((interval.min + interval.max) / 2));
  };
  ...
  const calc = useMemo(() => {
    ...
    let base = method === "normative" ? normBase : method === "unit" ? area * unitRate : method === "labor" ? hours * hourRate : fixedPrice;
    ...
    const p = stageP ? adjusted * .6 * pScope * (method === "normative" ? effectiveKtim.p * index : 1) : 0;
    const r = stageR ? adjusted * .4 * rScope * (method === "normative" ? effectiveKtim.r * index : 1) : 0;
  ...
  }, [area, complexity, effectiveKtim, fixedPrice, hourRate, hours, index, method, pricing, repeat, sections, sketch, stageP, stageR, unitRate, urgency, vat, works]);
